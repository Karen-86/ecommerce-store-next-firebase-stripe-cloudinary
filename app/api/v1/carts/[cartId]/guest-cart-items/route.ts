...
export async function POST(req: NextRequest, { params }: { params: Promise<{ cartId: string }> }) {
  try {
   ...
   const guestCartItems = body.guestCartItems.filter((guestCartItem: any) => {
      return !cart.items.some((cartItem: any) => {
        return guestCartItem.variantKey === cartItem.variantKey && guestCartItem.productId === cartItem.productId;
      });
    });
    ...
    const updatedCart = {
      ...
      items: [
        ...cart.items,
        ...guestCartItems.map((guestCartItem: any) => {
          return {
            id: uuidv4(),
